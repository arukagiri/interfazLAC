...
char serialib::ReadChar(char *pByte,unsigned int TimeOut_ms)
{
    ...
    DWORD dwBytesRead = 0;
    timeouts.ReadIntervalTimeout=MAXULONG_PTR;
    timeouts.ReadTotalTimeoutMultiplier=0;
    timeouts.ReadTotalTimeoutConstant=0;                                // Set the TimeOut
    if(!SetCommTimeouts(hSerial, &timeouts))                          // Write the parameters
      return -2;                                                      // Error while writing the parameters
    if(!ReadFile(hSerial,pByte, 1, &dwBytesRead, NULL))                 // Read the byte
        return -1;                                                      // Error while reading the byte
    if (dwBytesRead==0) return 0;                                       // Return 0 if the timeout is reached (no hay dato)
    return 1;                                                           // Success
#endif
#ifdef __linux__
